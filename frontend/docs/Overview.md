...
2. **Khảo sát tồn kho tự động:**
   ...
   - Đọc và cập nhật tồn kho trực tiếp trong PostgreSQL thông qua màn hình Site & Stock Registry.
   - Ghi nhận và lưu thông tin phản hồi vào **Inventory Database**.
...
5. **Chốt đơn (Purchase Order Generation):**
   ...
   - Xuất PO để gửi đến các Site đã được lựa chọn qua kênh nghiệp vụ nội bộ.
...
Để mang lại trải nghiệm làm việc hiện đại, chuyên nghiệp và hiệu quả, ứng dụng sử dụng JavaFX desktop client với các module chính:
...
Để đảm bảo tốc độ phản hồi nhanh, giao diện ổn định và dễ vận hành trong môi trường nội bộ, hệ thống được xây dựng bằng:

- **Desktop Client:** JavaFX chạy như app desktop, không phải web app.
- **Styling & UI Components:** JavaFX CSS kết hợp các control native như `TableView`, `Dialog`, `TabPane`, `SplitPane`, `DatePicker`, `ComboBox`.
- **Data Access:** JDBC trực tiếp bằng `DriverManager`, `Connection`, `PreparedStatement` và transaction SQL thủ công; không dùng REST client, không dùng ORM.
- **Data Storage:** Lưu trữ dữ liệu cấu hình Site, tồn kho, yêu cầu nhập hàng và PO trong **PostgreSQL**.
