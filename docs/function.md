...
*   **Hệ thống Menu điều hướng (Dynamic Navigation):**
    *   Người dùng thuộc OOAS được điều hướng vào menu nghiệp vụ của hệ thống đặt hàng quốc tế.
    *   Tài khoản thuộc Bộ phận Kho không vào sidebar OOAS; sau đăng nhập được chuyển thẳng sang giao diện Hệ thống quản lý kho độc lập.
...
## NHÓM 7 – HỆ THỐNG QUẢN LÝ KHO ĐỘC LẬP

Nhóm màn hình này thuộc hệ thống quản lý kho riêng. OOAS chỉ cung cấp dữ liệu PO để đối chiếu và nhận lại kết quả kiểm hàng; các màn hình không xuất hiện trên sidebar OOAS.

### MH14. Màn hình Danh sách đơn hàng cần nhập kho
*   **Mục đích:** Cung cấp cho Bộ phận Kho danh sách các PO đang vận chuyển hoặc đã đến kho để thực hiện quy trình kiểm hàng đối chiếu.
*   **Trường thông tin hiển thị (Bảng danh sách):**
...
*   **Thao tác điều khiển:**
    *   Nút **Kiểm hàng & xác nhận nhập kho**: Mở màn hình đối chiếu `MH15`.
    *   Nút **Xem chi tiết**: Xem thông tin PO và danh sách hàng đặt.

---

### MH15. Màn hình Kiểm tra & xác nhận nhập kho
*   **Mục đích:** Bộ phận kho đối chiếu số lượng hàng thực tế nhận được so với số lượng đặt trên PO và phát hiện chênh lệch.
...
*   **Thao tác điều khiển:**
    ...
    *   Nút **Xác nhận nhập kho**: Lưu kết quả đối chiếu và chuyển trạng thái PO thành `Hoàn tất`.
    *   Việc lưu và cập nhật tồn kho nội bộ tiếp tục được thực hiện trong hệ thống quản lý kho riêng, ngoài phạm vi dữ liệu tồn kho Site của OOAS.
